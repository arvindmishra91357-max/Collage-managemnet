...
async function uploadClassNote(req, res) {
  try {
    ...
    realtime.broadcastEvent({
      ...
      unit,
      action: 'UPLOAD',
      message: `📥 New Notes: ${subject} - ${title}`
    });
...
async function uploadStudyMaterial(req, res) {
  try {
    ...
    realtime.broadcastEvent({
      ...
      category,
      action: 'UPLOAD',
      message: `📚 Study Material Added: ${subject} - ${title}`
    });
...
async function createAssignment(req, res) {
  try {
    ...
    realtime.broadcastEvent({
      ...
      due_date,
      action: 'CREATE',
      message: `📝 New Assignment: ${subject} - ${title} (Due: ${due_date})`
    });
...
async function uploadQuestionPaper(req, res) {
  try {
    ...
    realtime.broadcastEvent({
      ...
      exam_name,
      action: 'UPLOAD',
      message: `📄 PYQ / Question Paper Uploaded: ${subject} (${exam_name})`
    });
...
async function deleteQuestionPaper(req, res) {
  try {
    ...
    realtime.broadcastEvent({ type: 'PYQ_UPDATED', action: 'DELETE' });
...
async function addCalendarEvent(req, res) {
  try {
    ...
    realtime.broadcastEvent({ type: 'CALENDAR_UPDATED', title, start_date });
...
async function deleteCalendarEvent(req, res) {
  try {
    ...
    realtime.broadcastEvent({ type: 'CALENDAR_UPDATED', action: 'DELETE' });
...
async function deleteAnnouncement(req, res) {
  try {
    ...
    realtime.broadcastEvent({ type: 'NOTICES_UPDATED', action: 'DELETE' });
