...
async function getAllStudents(req, res) {
  try {
    ...
    let sql = "SELECT id, ug_id, name, roll_number, phone_number, batch, program, year, semester, division, academic_year, profile_photo_url, is_cr, cr_batches, status, created_at FROM students WHERE 1=1";
    const params = [];
...
async function getStudentById(req, res) {
  try {
    ...
    const student = await db.get(
      "SELECT id, ug_id, name, roll_number, phone_number, batch, program, year, semester, division, academic_year, profile_photo_url, is_cr, cr_batches, status, created_at FROM students WHERE id = ? OR ug_id = ?",
      [id, id]
    );
...
async function updateStudent(req, res) {
  try {
    const { id } = req.params;
    const { name, roll_number, phone_number, password, status, is_cr, cr_batches } = req.body;
    ...
    const updatedCrBatches = cr_batches !== undefined ? cr_batches : (student.cr_batches || 'Both');
    ...
    await db.run(`
      UPDATE students
      SET name = ?, roll_number = ?, phone_number = ?, batch = ?, status = ?, is_cr = ?, cr_batches = ?, password_hash = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `, [updatedName, rollNum, updatedPhone, updatedBatch, updatedStatus, updatedCr, updatedCrBatches, updatedPassHash, student.id]);
...
async function toggleCRStatus(req, res) {
  try {
    const { id } = req.params;
    const { is_cr, cr_batches } = req.body;

    const student = await db.get("SELECT * FROM students WHERE id = ? OR UPPER(ug_id) = UPPER(?)", [id, id]);
    if (!student) {
      return res.status(404).json({ success: false, message: 'Student not found.' });
    }

    const newCrStatus = is_cr !== undefined ? (is_cr == 1 || is_cr === true || is_cr === '1' ? 1 : 0) : (student.is_cr === 1 ? 0 : 1);
    const newCrBatches = newCrStatus === 1 ? (cr_batches || student.cr_batches || student.batch || 'Both') : 'None';

    await db.run("UPDATE students SET is_cr = ?, cr_batches = ? WHERE id = ?", [newCrStatus, newCrBatches, student.id]);

    // Dispatch Real-Time SSE broadcast so student portal updates immediately
    try {
      realtime.broadcastEvent({
        type: 'CR_STATUS_UPDATED',
        student_id: student.id,
        ug_id: student.ug_id,
        name: student.name,
        is_cr: newCrStatus,
        cr_batches: newCrBatches
      });
    } catch (e) {}

    return res.json({
      success: true,
      message: `Student ${student.name} is now ${newCrStatus === 1 ? `designated as Class Representative (CR) for ${newCrBatches}` : 'removed from Class Representative (CR)'}.`,
      is_cr: newCrStatus,
      cr_batches: newCrBatches
    });
  } catch (err) {
