...
const AdminApp = {
            ...
            <tbody>
              ${students.map(s => `
                <tr data-name="${s.name.toLowerCase()}" data-ugid="${s.ug_id.toLowerCase()}">
                  ...
                  <td>
                    ${s.is_cr === 1 ? `<span class="lab-chip" style="background:rgba(245,158,11,0.2); color:#fbbf24; border:1px solid rgba(245,158,11,0.4);">👑 CR (${s.cr_batches || 'Both'})</span>` : '<span style="font-size:12px; color:var(--text-muted);">Student</span>'}
                  </td>
                  ...
                  <td>
                    <button class="icon-btn" onclick="AdminApp.openManageCRModal(${s.id}, '${s.name.replace(/'/g, "\\'")}', ${s.is_cr ? 1 : 0}, '${s.cr_batches || 'Both'}')" title="Manage CR Role & Assigned Batches" style="width:30px; height:30px; display:inline-flex; align-items:center; justify-content:center; color:${s.is_cr === 1 ? '#fbbf24' : 'var(--text-muted)'}; background:${s.is_cr === 1 ? 'rgba(245,158,11,0.15)' : 'rgba(255,255,255,0.05)'}; border:1px solid ${s.is_cr === 1 ? 'rgba(245,158,11,0.4)' : 'var(--border-color)'}; border-radius:6px; margin-right:4px;">👑</button>
                    <button class="icon-btn" onclick="AdminApp.openEditStudentModal(${s.id}, '${s.name.replace(/'/g, "\\'")}', '${s.ug_id}', ${s.roll_number}, ${s.is_cr ? 1 : 0}, '${s.cr_batches || 'Both'}')" title="Edit" style="width:28px; height:28px; display:inline-flex;">✎</button>
                    <button class="icon-btn" onclick="AdminApp.deleteStudentPrompt(${s.id}, '${s.name.replace(/'/g, "\\'")}')" title="Delete" style="width:28px; height:28px; display:inline-flex; color:#f87171;">🗑</button>
                  </td>
                </tr>
              `).join('')}
            </tbody>
  ...
  openManageCRModal(id, name, currentCr, currentBatches = 'Both') {
    const modalContainer = document.getElementById('admin-modal-container') || document.body;
    let existing = document.getElementById('manage-cr-modal');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'manage-cr-modal';
      ...
      <div class="modal-card" style="max-width:460px; width:95%;">
        <div class="modal-header">
          <div style="display:flex; align-items:center; gap:8px;">
            ...
            <h3 style="font-size:16px; font-weight:800;">CR Management: ${name}</h3>
          </div>
          <button class="icon-btn" onclick="document.getElementById('manage-cr-modal').remove()" style="width:30px; height:30px;">✕</button>
        </div>
        <div class="modal-body">
          <p style="font-size:12.5px; color:var(--text-secondary); margin-bottom:14px;">
            Assign CR authorization and permissions for <strong>${name}</strong>:
          </p>

          <div class="form-group">
            <label class="form-label">CR Role Status</label>
            <select id="cr-status-select" class="form-control" onchange="document.getElementById('cr-batch-group').style.display = this.value === '1' ? 'block' : 'none';">
              <option value="1" ${currentCr === 1 ? 'selected' : ''}>Active Class Representative (CR)</option>
              <option value="0" ${currentCr !== 1 ? 'selected' : ''}>Not a CR (Regular Student)</option>
            </select>
          </div>

          <div class="form-group" id="cr-batch-group" style="${currentCr === 1 ? '' : 'display:none;'}">
            <label class="form-label">Assigned Batches (Permissions Scope) *</label>
            <select id="cr-batches-select" class="form-control">
              <option value="Batch 1" ${currentBatches === 'Batch 1' ? 'selected' : ''}>Batch 1 Only (Roll 1–30)</option>
              <option value="Batch 2" ${currentBatches === 'Batch 2' ? 'selected' : ''}>Batch 2 Only (Roll 31+)</option>
              <option value="Both" ${currentBatches === 'Both' ? 'selected' : ''}>Batch 1 + Batch 2 (Both Batches)</option>
            </select>
            <p style="font-size:11px; color:var(--text-muted); margin-top:4px;">
              ℹ️ A CR assigned to "Batch 1 + Batch 2" can manage both batches independently without cross-batch interference.
            </p>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn-primary" style="width:auto; background:var(--bg-input); border:1px solid var(--border-color); color:var(--text-secondary); margin-top:0;" onclick="document.getElementById('manage-cr-modal').remove()">Cancel</button>
          <button class="btn-primary" style="width:auto; margin-top:0; background:linear-gradient(135deg, #d97706, #b45309); border-color:#f59e0b;" onclick="AdminApp.submitCRManagement(${id})">Save CR Permissions</button>
        </div>
      </div>
    `;
  ...
  async submitCRManagement(id) {
    const is_cr = parseInt(document.getElementById('cr-status-select').value, 10);
    const cr_batches = is_cr === 1 ? document.getElementById('cr-batches-select').value : 'None';

    const res = await API.toggleCR(id, is_cr, cr_batches);
    if (res.success) {
      window.App.showToast(res.message, 'success');
      const modal = document.getElementById('manage-cr-modal');
      if (modal) modal.remove();
      this.loadSectionData(this.currentSection);
    } else {
      window.App.showToast(res.message || 'Failed to update CR permissions.', 'error');
    }
  },
  ...
  openEditStudentModal(id, name, ug_id, roll, is_cr = 0, cr_batches = 'Both') {
    try {
      history.pushState({ role: 'ADMIN', modal: 'edit-student-modal' }, '', '#admin-students-edit');
    } catch (e) {}

    const modalContainer = document.getElementById('admin-modal-container') || document.body;
    let existing = document.getElementById('edit-student-modal');
    if (existing) existing.remove();
    ...
    modal.innerHTML = `
      <div class="modal-card" style="max-width:480px; width:95%;">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label class="form-label">CR Authorization Status</label>
            <select id="edit-is-cr" class="form-control" onchange="document.getElementById('edit-cr-batches-group').style.display = this.value === '1' ? 'block' : 'none';">
              <option value="0" ${is_cr !== 1 ? 'selected' : ''}>Regular Student</option>
              <option value="1" ${is_cr === 1 ? 'selected' : ''}>👑 Class Representative (CR)</option>
            </select>
          </div>
          <div class="form-group" id="edit-cr-batches-group" style="${is_cr === 1 ? '' : 'display:none;'}">
            <label class="form-label">Assigned Batches (Permissions Scope)</label>
            <select id="edit-cr-batches" class="form-control">
              <option value="Batch 1" ${cr_batches === 'Batch 1' ? 'selected' : ''}>Batch 1 Only (Roll 1–30)</option>
              <option value="Batch 2" ${cr_batches === 'Batch 2' ? 'selected' : ''}>Batch 2 Only (Roll 31+)</option>
              <option value="Both" ${cr_batches === 'Both' ? 'selected' : ''}>Batch 1 + Batch 2 (Both Batches)</option>
            </select>
          </div>
          <div class="form-group">
  ...
  async submitEditStudent(id) {
    ...
    const is_cr = parseInt(document.getElementById('edit-is-cr').value, 10);
    const cr_batches = is_cr === 1 ? document.getElementById('edit-cr-batches').value : 'None';
  ...
  promptSaveAdminQuickAttendance() {
    ...
    if (this._adminQuickModalEscHandler) {
      document.removeEventListener('keydown', this._adminQuickModalEscHandler);
    }
    this._adminQuickModalEscHandler = (e) => {
      if (e.key === 'Escape') AdminApp.closeAdminQuickConfirmModal();
    };
  ...
  closeAdminQuickConfirmModal() {
    if (this._adminQuickModalEscHandler) {
      document.removeEventListener('keydown', this._adminQuickModalEscHandler);
      this._adminQuickModalEscHandler = null;
    }
...
<body>
  ...
  openRoomChangeModal(timetableId, subject, startTime, endTime, currentRoom, currentBatch = 'Both') {
    const targetDate = this.selectedAdminDate || this.getTodayDateString();
    ...
    modalContainer.innerHTML = `
      <div class="modal-backdrop" id="room-change-modal">
        <div class="modal-card">
          <div class="modal-header">
            <h3 style="font-size:16px; font-weight:800;">🔄 Room Change / Schedule Edit: ${subject}</h3>
            <button class="icon-btn" onclick="AdminApp.closeModal('room-change-modal')" style="width:30px; height:30px;">✕</button>
          </div>
          <div class="modal-body">
            <div style="background:var(--bg-input); padding:12px; border-radius:var(--radius-sm); margin-bottom:14px; font-size:13px; border:1px solid var(--border-color);">
            ...
            <div class="form-group">
              <label class="form-label">Target Batch *</label>
              <select id="rc-batch-select" class="form-control">
                <option value="Both" ${currentBatch === 'Both' ? 'selected' : ''}>Both Batches (Batch 1 + Batch 2)</option>
                <option value="Batch 1" ${currentBatch === 'Batch 1' ? 'selected' : ''}>Batch 1 Only (Roll 1–30)</option>
                <option value="Batch 2" ${currentBatch === 'Batch 2' ? 'selected' : ''}>Batch 2 Only (Roll 31+)</option>
              </select>
            </div>

            <div class="form-group">
              <label class="form-label">Change Type *</label>
              <select id="rc-change-type" class="form-control">
                <option value="TODAY" selected>Only for Target Date (Temporary Override)</option>
            ...
            <div class="form-group">
              ...
              <input type="text" id="rc-reason" class="form-control" placeholder="e.g. Lab shift, Projector issue, Room 203 assigned" required />
            </div>

            <div style="margin-top:10px;">
              <label style="display:flex; align-items:center; gap:8px; font-size:12px; cursor:pointer; color:#fbbf24;">
                <input type="checkbox" id="rc-force-conflict" />
                <span>🛡️ Force Override Conflict (Admin Privilege)</span>
              </label>
            </div>
          </div>
          <div class="modal-footer">
  ...
  async submitRoomChangeOverride(timetableId, targetDate) {
    ...
    const batch = document.getElementById('rc-batch-select').value;
    const change_type = document.getElementById('rc-change-type').value;
    const override_conflict = document.getElementById('rc-force-conflict').checked;
                    ...
                    <tr>
                      ...
                      <td>
                        <span class="lab-chip" style="background:${h.action === 'ROOM_CHANGE' ? 'rgba(56,189,248,0.2)' : h.action === 'PERMANENT_CHANGE' ? 'rgba(168,85,247,0.2)' : h.action === 'CANCELLED' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.2)'}; color:${h.action === 'ROOM_CHANGE' ? '#38bdf8' : h.action === 'PERMANENT_CHANGE' ? '#c084fc' : h.action === 'CANCELLED' ? '#f87171' : '#34d399'};">
                          ${h.action === 'ROOM_CHANGE' ? 'Temporary Room Change' : h.action === 'PERMANENT_CHANGE' ? 'Permanent Change' : h.action}
                        </span>
                      </td>
                      <td>
                        ${(h.action === 'ROOM_CHANGE' || h.action === 'PERMANENT_CHANGE') ? `
